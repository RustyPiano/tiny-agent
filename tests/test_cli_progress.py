...
def test_emit_tool_detail_run_job_shows_operation_status_and_activity():
    events: list[str] = []
    runtime = _make_runtime(_NoopProvider(), _RecordingRegistry(), ui_event_printer=events.append)

    runtime._emit_tool_detail(
        "run_job",
        {"operation": "status", "job_id": "job_123"},
        '{"job_id":"job_123","status":"running","activity":"quiet","recommended_poll_after_s":5,"output_tail":"line-1"}',
    )

    assert any(
        "job=" in e and "op=status" in e and "status=running" in e and "activity=quiet" in e
        and "next=5" in e and "line-1" in e
        for e in events
    )


def test_emit_tool_detail_run_job_shows_exit_code_for_terminal_result():
    events: list[str] = []
    runtime = _make_runtime(_NoopProvider(), _RecordingRegistry(), ui_event_printer=events.append)
    ...
    runtime._emit_tool_detail(
        "run_job",
        {"operation": "status", "job_id": "job_123"},
        '{"job_id":"job_123","status":"succeeded","activity":"quiet","terminal":true,"exit_code":0,"recommended_poll_after_s":null}',
    )

    assert any(
        "job=" in e
        and "status=succeeded" in e
        and "exit=0" in e
        for e in events
    )


def test_emit_tool_detail_run_job_shows_cancellation_result():
    events: list[str] = []
    runtime = _make_runtime(_NoopProvider(), _RecordingRegistry(), ui_event_printer=events.append)

    runtime._emit_tool_detail(
        "run_job",
        {"operation": "cancel", "job_id": "job_123"},
        '{"job_id":"job_123","status":"cancelled","activity":"quiet","terminal":true,"recommended_poll_after_s":null}',
    )

    assert any(
        "job=" in e and "op=cancel" in e and "status=cancelled" in e
        for e in events
    )
...
def test_emit_tool_detail_job_tools_fall_back_when_json_parse_fails():
    ...
    runtime._emit_tool_detail("run_job", {"operation": "status"}, "plain text output")

    assert any("状态=ok" in e and "plain text output" in e for e in events)


def test_emit_tool_detail_run_job_falls_back_on_malformed_and_non_dict_json():
    events: list[str] = []
    runtime = _make_runtime(_NoopProvider(), _RecordingRegistry(), ui_event_printer=events.append)

    runtime._emit_tool_detail("run_job", {"operation": "status", "job_id": "job_123"}, "{bad json")
    runtime._emit_tool_detail(
        "run_job",
        {"operation": "status", "job_id": "job_123"},
        '["not", "dict"]',
...
def test_emit_tool_detail_job_tools_include_error_context_on_failure_status():
    ...
    runtime._emit_tool_detail(
        "run_job",
        {"operation": "status", "job_id": "job_123"},
        '{"job_id":"job_123","status":"failed","terminal":true,"exit_code":1,"message":"process crashed"}',
    )

    assert any("error=" in e and "process crashed" in e for e in events)
