...
4. `run_bash` 仅用于前台短任务和快速探查；预计可能超过短任务窗口的命令必须使用 `run_job`。
5. 训练、构建、长测试、长脚本执行等长任务默认使用 `run_job` 的 `start/status/cancel` 流程。
6. `run_bash` 超时后，不要通过增大 `timeout` 重试；应改用 `run_job`。
